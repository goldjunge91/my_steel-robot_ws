...
void PubEntities::run()
{
  ...
  for (;;)
  {
    if (xCount != 0)
    {
        ...
        if (cmd.cmd == TABLE_POSE)
        {
          ...
          if (cw < ccw)
          {
            ...
            else
            {
              xSpeed = (cw / dif);
              printf(
                  "Move from %f to %f (distance %f) at %f rads per sec Time to move "
                  "%f\n",
      ...
      if (!uRosBridge::getInstance()->publish(&xPublisher, &msg, this, NULL))
      {
      ...
      joint_state_msg.position.data[0] = a;
      if (!uRosBridge::getInstance()->publish(&xPubJoint, &joint_state_msg, this, NULL))
      {
...
{
  printf("[PubEntities] Creating entities...\n");

  rclc_publisher_init_default(&xPublisher, node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "pico_rnd");
  printf("[PubEntities]   ✅ Publisher: pico_rnd (std_msgs/Int32)\n");

  rclc_publisher_init_default(&xPubJoint, node, ROSIDL_GET_MSG_TYPE_SUPPORT(sensor_msgs, msg, JointState),
                              "joint_states");
  printf("[PubEntities]   ✅ Publisher: joint_states (sensor_msgs/JointState)\n");

  rclc_subscription_init_default(&xSubTwist, node, ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, Twist), "cmd_vel");
  printf("[PubEntities]   ✅ Subscriber: cmd_vel (geometry_msgs/Twist)\n");

  rclc_subscription_init_default(&xSubPose, node, ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, PoseStamped),
                                 "do_pose");
  printf("[PubEntities]   ✅ Subscriber: do_pose (geometry_msgs/PoseStamped)\n");

  xCount = 4;
  printf("[PubEntities] ✅ Total: %d entities created\n", xCount);
}
...
{
  printf("[PubEntities] Destroying entities...\n");
  xCount = 0;
  ...
  printf("[PubEntities] ✅ All entities destroyed\n");
}
...
void PubEntities::addToExecutor(rclc_executor_t* executor)
{
  ...
  rclc_executor_add_subscription_with_context(executor, &xSubTwist, &xTwistMsg, uRosEntities::subscriptionCallback,
                                              &xSubTwistContext, ON_NEW_DATA);

  rclc_executor_add_subscription_with_context(executor, &xSubPose, &xPoseMsg, uRosEntities::subscriptionCallback,
                                              &xSubPoseContext, ON_NEW_DATA);
}
