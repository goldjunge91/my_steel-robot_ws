...
void PubEntities::createEntities(rcl_node_t *node, rclc_support_t *support) {
    rclc_publisher_init_default(&xPublisher, node,
                                ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "pico_rnd");

    rclc_publisher_init_default(&xPubJoint, node,
...
void PubEntities::addToExecutor(rclc_executor_t *executor) {
    ...
    rclc_executor_add_subscription_with_context(executor, &xSubTwist, &xTwistMsg,
                                                uRosEntities::subscriptionCallback,
                                                &xSubTwistContext, ON_NEW_DATA);

    rclc_executor_add_subscription_with_context(executor, &xSubPose, &xPoseMsg,
                                                uRosEntities::subscriptionCallback,
