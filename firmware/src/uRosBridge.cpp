...
void uRosBridge::run() {
    ...
    for (;;) {
        ...
        if (state != last_state) {
            uart_log("[STATE] Changed from %d to %d\r\n", (int)last_state, (int)state);
            last_state = state;
        }
                    ...
                    if (!pingAgent()) {
                        if (xPingFailures < UROS_MAX_PING_FAILURES) {
                            ...
                            uart_log("Ping timeout %u/%u\r\n",
                                     (unsigned)xPingFailures,
                                     (unsigned)UROS_MAX_PING_FAILURES);
                        } else {
                            xPingFailures = 0;
                ...
                if (xPubQ != NULL) {
                    ...
                    while (res == pdTRUE) {
                        ...
                        if (res == pdTRUE) {
                            ...
                            // Diagnostic: log attempt and result of rcl_publish
                            uart_log("[PUB] Attempting publish...\r\n");
                            rcl_ret_t pubRet = rcl_publish(cmd.publisher, cmd.msg, NULL);
                            uart_log("[PUB] Result: %d\r\n", (int)pubRet);
                            if (RCL_RET_OK != pubRet) {
                                const char *err = rcl_get_error_string().str;
                                uart_log("[PUB] rcl error: %s\r\n", err != NULL ? err : "(null)");
                                rcl_reset_error();
...
bool uRosBridge::createEntities() {
    ...
    rclc_publisher_init_default(
        &xPublisher, &xNode, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "pico_count");
...
bool uRosBridge::publish(rcl_publisher_t *publisher,
    ...
    if (!xSessionReady) {
        uart_log("[uRosBridge] WARN: Session not ready, clearing message\r\n");
        if (entities != NULL) {
            entities->pubComplete(msg, args, PubCleared);
        }
        return true;
