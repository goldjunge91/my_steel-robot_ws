...
docker buildx build --platform linux/arm64 -f docker/Dockerfile.robot-pi.test -t robot:pi-test-local . --load
...
# Build and push the production Docker image with logging
...
# Alternative: Build with multiple tags and logging
...
# Check if you're logged into Docker Hub first
docker login

# Monitor the build progress in real-time (run in separate terminal)
tail -f docker-build-production.log

# After build completes, verify the image was pushed
docker buildx imagetools inspect goldjunge491/my-steel-robot:humble-arm64

# Test the image locally (if you want to verify before Pi deployment)
